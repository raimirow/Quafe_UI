...
local function Border2_PowerNum_Update(frame, per, cur, power_type)
	local value
...
local function DeathKnight_Event(frame, event, ...)
	if event == "PLAYER_LOGIN" then
		...
		DeathKnight_CombatLog.LogEvent = function(timestamp, eventParam, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, Param12, Param13, Param14, Param15, Param16, Param17, Param18, Param19, Param20, Param21, Param22, Param23, Param24)
			...
			if (band(destFlags, COMBATLOG_FILTER_ME) == COMBATLOG_FILTER_ME) then
				local damage = 0
		...
		F.BGCL_Toggle()
		frame.Bar5: Show()
		frame.Bar5.Ring1A: SetVertexColor(F.Color(PowerColor.Damage1))
		frame.Bar5.Ring1B: SetVertexColor(F.Color(PowerColor.Damage2))
		frame.Bar5.Bg2: Hide()
		frame.Bar5.Bd3: Hide()
	end
end
...
local function DeathKnight_Update(frame, elapsed)
	...
	if F.Last25 == 0 then
		if (Damage2 + Damage3) > 0 then
			frame.Bar5: SetAlpha(1)
			if (not frame.Damage2 or frame.Damage2 ~= Damage2) or (not frame.Damage3 or frame.Damage3 ~= Damage3) then
				...
				frame.Damage2 = Damage2
				frame.Damage3 = Damage3
			end
		else
			frame.Bar5: SetAlpha(0)
		end
...
local function Threat_Update(frame)
	if UnitAffectingCombat("player") and UnitExists("target") and not UnitIsDeadOrGhost("target") then
		if UnitCanAttack("player", "target") and not (UnitIsDead("target") or UnitIsFriend("player", "target") or UnitPlayerControlled("target")) then
			...
			if status and threatpct then
				threatpct = threatpct/100
				if not frame.Bar5.Shown then
                    frame.Bar5.Shown = true
				    frame.Bar5: SetAlpha(1)
                end
				frame.Bar5.Ring1A: SetVertexColor(F.Color(GetThreatColor(status,frame.IsTank)))
				LoopBar5_Update(frame, threatpct)
			else
...
local MEKA_LOOPHUD = F.IsClassic and CreateFrame("Frame", nil, E) or CreateFrame("Frame", nil, E, "MEKA_LoopHUD_SlideTemplate")
...
local function MEKA_LoopHUD_Player_HpUpdate(arg1,arg2,arg3)
	if arg1 == "Update" then
		LoopBar1_Update(MEKA_LOOPHUD.Left, arg2)
    elseif arg1 == "Event" then
		Num48_Update(MEKA_LOOPHUD.Left, arg2)
    end
end

local function MEKA_LoopHUD_Player_PpUpdate(arg1,arg2,arg3,arg4,arg5)
	if arg1 == "Update" then
		LoopBar2_Update(MEKA_LOOPHUD.Left, arg2)
		if MEKA_LOOPHUD.Left.Insane then
			LoopBar5_Update(MEKA_LOOPHUD.Left, arg2, true)
		end
    elseif arg1 == "Event" then
		if SHOW_POWERNUM then
			Border2_PowerNum_Update(MEKA_LOOPHUD.Left, arg2,arg3,arg5)
		end
    end
end

local function MEKA_LoopHUD_Player_AsUpdate(arg1,arg2)
	if arg1 == "Update" then
		LoopBar4_Update(MEKA_LOOPHUD.Left, arg2)
	end
end

local function MEKA_LoopHUD_Player_MnUpdate(arg1,arg2)
	if arg1 == "Update" then
		LoopBar3_Update(MEKA_LOOPHUD.Left, arg2)
	end
end

local function MEKA_LoopHUD_Pet_HpUpdate(arg1,arg2)
	if arg1 == "Update" then
		LoopBar6_Update(MEKA_LOOPHUD.Left, arg2)
	end
end

local function MEKA_LoopHUD_Pet_PpUpdate(arg1,arg2)
	if arg1 == "Update" then
		LoopBar6_Update(MEKA_LOOPHUD.Left,nil, arg2)
	end
end

local function MEKA_LoopHUD_Target_HpUpdate(arg1,arg2,arg3)
	if arg1 == "Update" then
		LoopBar1_Update(MEKA_LOOPHUD.Right, arg2)
    elseif arg1 == "Event" then
		Num48_Update(MEKA_LOOPHUD.Right, arg2)
    end
end

local function MEKA_LoopHUD_Target_PpUpdate(arg1,arg2)
	if arg1 == "Update" then
		LoopBar3_Update(MEKA_LOOPHUD.Right, arg2)
    elseif arg1 == "Event" then
		
    end
end

local function MEKA_LoopHUD_Target_AsUpdate(arg1,arg2)
	if arg1 == "Update" then
		LoopBar4_Update(MEKA_LOOPHUD.Right, arg2)
	end
end
...
local function MEKA_LOOPHUD_Load()
	...
	if Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_HUD.State == "LOOP" then
		...
		tinsert(E.UBU.Player.HP, MEKA_LoopHUD_Player_HpUpdate)
        tinsert(E.UBU.Player.PP, MEKA_LoopHUD_Player_PpUpdate)
        tinsert(E.UBU.Player.AS, MEKA_LoopHUD_Player_AsUpdate)
        tinsert(E.UBU.Player.MN, MEKA_LoopHUD_Player_MnUpdate)
		tinsert(E.UBU.Pet.HP, MEKA_LoopHUD_Pet_HpUpdate)
		tinsert(E.UBU.Pet.PP, MEKA_LoopHUD_Pet_PpUpdate)

		RightFrame_Create(MEKA_LOOPHUD)
		tinsert(E.UBU.Target.HP, MEKA_LoopHUD_Target_HpUpdate)
        tinsert(E.UBU.Target.PP, MEKA_LoopHUD_Target_PpUpdate)
        tinsert(E.UBU.Target.AS, MEKA_LoopHUD_Target_AsUpdate)
