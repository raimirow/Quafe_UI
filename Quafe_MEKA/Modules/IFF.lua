...
local function Castbar1_Artwork(frame)
	...
	local Txt = F.Create.Font(frame, "OVERLAY", C.Font.Txt, 12, nil, nil,nil, {r=0,g=0,b=0},0, {0,0}, "LEFT", "CENTER")
	Txt: SetSize(90,8)
	Txt: SetPoint("LEFT", Icon, "RIGHT", 4,-1)
	Txt: SetTextColor(F.Color(C.Color.Main0))

	local Num = F.Create.Font(frame, "OVERLAY", C.Font.Num, 12, nil, nil,nil, {r=0,g=0,b=0},0, {0,0}, "RIGHT", "CENTER")
...
local function Target_Name(frame)
	...
	local Level = F.create_Font(frame, C.Font.Num, 12, nil, 0)
	Level: SetPoint("TOPLEFT", Name, "BOTTOMLEFT", -1,-6)
	Level: SetAlpha(1)

	local Num = F.create_Font(frame, C.Font.Num, 12, nil, 0)
...
local function Target_Aura(frame)
	...
	frame.Aura.limit = 20
	frame.Aura.bufflimit = 10
	frame.Aura.debufflimit = 10
...
local function GetRealUnitHealth(unit)
	local MinHealth,MaxHealth
	if F.IsClassic and RealMobHealth then
		MinHealth,MaxHealth = RealMobHealth.GetUnitHealth(unit)
	else
		MinHealth,MaxHealth = UnitHealth(unit),UnitHealthMax(unit)
	end
...
local function Target_Event(frame, event, ...)
	...
	if event == "PLAYER_ENTERING_WORLD" or event == "PLAYER_TARGET_CHANGED" or event == "UNIT_HEALTH" or event == "UNIT_MAXHEALTH"  or event == "UNIT_POWER_UPDATE" or event == "UNIT_MAXPOWER" then
		local MinHealth,MaxHealth = GetRealUnitHealth(frame.Unit)
...
local function Target_Frame(frame)
	local Target = CreateFrame("Button", "Quafe_MEKA "..L['TARGET_FRAME'], frame, "SecureUnitButtonTemplate")
	Target.Unit = "target"
	...
	local PowerBarAlt = TargetFramePowerBarAlt
	if PowerBarAlt then
		PowerBarAlt: SetParent(Target)
		PowerBarAlt: ClearAllPoints()
		PowerBarAlt: SetPoint("TOP", Target, "BOTTOM", -16,-54)
	end
...
local function TargetTarget_Aura(frame)
	...
	frame.Aura.perline = 5
	frame.Aura.growthH = "RIGHT"
	frame.Aura.growthV = "BOTTOM"
	frame.Aura.unit = frame.Unit
	frame.Aura.type = nil
	frame.Aura.filter = "HARMFUL"
	
	frame.Aura: SetSize(frame.Aura.size*frame.Aura.perline, frame.Aura.size)
	frame.Aura: SetPoint("TOP", frame, "BOTTOM", -4,-2)
...
local function TargetTarget_Frame(frame)
	local TargetTarget = CreateFrame("Button", "Quafe_MEKA "..L['TARGET_TARGET_FRAME'], frame, "SecureUnitButtonTemplate")
	...
	TargetTarget_Aura(TargetTarget)
...
local function Focus_Frame(frame)
	local Focus = CreateFrame("Button", "Quafe_MEKA "..L['FOCUS_FRAME'], frame, "SecureUnitButtonTemplate")
	Focus.Unit = "focus"
...
local function FocusTarget_Frame(frame)
	local FocusTarget = CreateFrame("Button", "Quafe_MEKA "..L['FOCUS_TARGET_FRAME'], frame, "SecureUnitButtonTemplate")
...
F.IFF_Castbar_Toggle = function(arg1,arg2)
	if F.IsClassic then
		if not (MEKA_IFF.Target) then return end
		if arg1 == "ON" then
			MEKA_IFF.Target.CastBar.EnableCastbar = true
		elseif arg1 == "OFF" then
			MEKA_IFF.Target.CastBar.EnableCastbar = false
		end
	else
		if not (MEKA_IFF.Target and MEKA_IFF.Focus) then return end
		if arg1 == "ON" then
			MEKA_IFF.Target.CastBar.EnableCastbar = true
			MEKA_IFF.Focus.CastBar.EnableCastbar = true
		elseif arg1 == "OFF" then
			MEKA_IFF.Target.CastBar.EnableCastbar = false
			MEKA_IFF.Focus.CastBar.EnableCastbar = false
		end
	end
end
...
local function Boss_Name(frame)
	local Name = F.Create.Font(frame, "OVERLAY", C.Font.Txt, 10, nil, nil,nil, C.Color.Main0,0.9, {1,1}, "LEFT", "CENTER")
	Name: SetSize(80,8)
...
local function Boss_Artwork(frame)
	...
	local Indicator = F.Create.Texture(frame, "ARTWORK", 1, MEKA.Path("IFF_Boss_Bd1"), C.Color.Main1, 0.25, {19,20}, {13/64,51/64,12/64,52/64})
	Indicator: SetPoint("RIGHT", frame, "RIGHT", -7,0)

	local HealthBar = F.Create.Texture(frame, "ARTWORK", 1, MEKA.Path("IFF_Boss_Bar1"), C.Color.Main1, 0.9, {142,16}, {114/512,398/512,0,1})
	...
	local PowerBar = F.Create.Texture(frame, "ARTWORK", 1, MEKA.Path("IFF_Boss_Bar1"), C.Color.Main1, 0.9, {142,16}, {114/512,398/512,1,0})
	...
	local Percent = F.Create.Font(frame, "OVERLAY", C.Font.Big, 18, nil, nil,nil, C.Color.Main0,0.9, {1,1}, "CENTER", "CENTER")
...
local function MEKA_BossFrame_Load()
	if Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_BossFrame.Enable then
		...
		for i = 1,MAX_BOSS_FRAMES do
			Boss[i] = CreateFrame("Button", "Quafe_MEKA "..L['BOSS_FRAME']..i, MEKA_BossFrame, "SecureUnitButtonTemplate")
			Boss[i].Unit = "boss"..i
			--Boss[i].Unit = "player"
