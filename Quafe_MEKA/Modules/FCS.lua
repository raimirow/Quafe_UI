...
local function Icon_Template(frame)
    ...
    local Bd = Main: CreateTexture(nil, "ARTWORK")
	...
	for k,v in pairs(filter) do
		if v and v.Show and NUM[v.Style] and NUM[v.Style] < NUM.Max then
			...
			if v.Spell then
				...
				local name = C_Spell.GetSpellName(v.Spell)
	...
	for k,v in pairs(filter) do
		if v and v.Show and NUM[v.Style] and NUM[v.Style] < NUM.Max then
			...
			if v.Spell then
				...
				if SpellInfo then
					frame.Button[frame.Info.NumButton].Info.SpellName = SpellInfo.name
					frame.Button[frame.Info.NumButton].Info.SpellID = SpellInfo.spellID
					frame.Button[frame.Info.NumButton].Info.IconID = SpellInfo.iconID
					frame.Button[frame.Info.NumButton].Info.SpellColor = v.SpellColor
					frame.Button[frame.Info.NumButton].Main.Icon: SetTexture(SpellInfo.iconID)
				end
			end
			if v.Aura then
				frame.Button[frame.Info.NumButton].Info.AuraID = v.Aura
				...
				--local name, rank, fileID, _,_,_, spellID = GetSpellInfo(v.Aura)
				local SpellInfo
				if type(v.Aura) == "table" then
					SpellInfo = C_Spell.GetSpellInfo(v.Aura[1])
				else
					SpellInfo = C_Spell.GetSpellInfo(v.Aura)
				end
				if fileID then
					frame.Button[frame.Info.NumButton].Main.Icon: SetTexture(SpellInfo.iconID)
				end
...
local function Watcher_AuraEvent(frame)
	...
	local AuraInfo, name, icon, count, dispelType, duration, expirationTime, unitCaster, canStealOrPurge, nameplateShowPersonal, spellID, canApplyAura, isBossDebuff, isCastByPlayer, nameplateShowAll, timeMod, value1, value2, value3
	for k, v in ipairs(UNIT_LIST) do
		local NumBuff,NumDebuff = 1, 1
		local b,d
		local isFriend = UnitIsFriend('player', v)
		if (v == "player" or v == "pet") or (not FCS_FILTER) or (FCS_FILTER and isFriend)then
			while NumBuff == 1 or b do
				...
				AuraInfo = F.UnitAura(v, NumBuff, "HELPFUL")
				if AuraInfo then
					b = true
					Watcher_AuraUpdate(frame.BarWatcher, v, AuraInfo.name, AuraInfo.icon, AuraInfo.charges, AuraInfo.duration, AuraInfo.expirationTime, AuraInfo.sourceUnit, AuraInfo.spellId)
					Watcher_AuraUpdate(frame.IconWatcher, v, AuraInfo.name, AuraInfo.icon, AuraInfo.charges, AuraInfo.duration, AuraInfo.expirationTime, AuraInfo.sourceUnit, AuraInfo.spellId)
				end
				NumBuff = NumBuff + 1
			end
		end
		if (v == "player" or v == "pet") or (not FCS_FILTER) or (FCS_FILTER and not isFriend)then
			while NumDebuff == 1 or d do
				...
				AuraInfo = F.UnitAura(v, NumDebuff, "HARMFUL")
				if AuraInfo then
					d = true
					Watcher_AuraUpdate(frame.BarWatcher, v, AuraInfo.name, AuraInfo.icon, AuraInfo.charges, AuraInfo.duration, AuraInfo.expirationTime, AuraInfo.sourceUnit, AuraInfo.spellId)
					Watcher_AuraUpdate(frame.IconWatcher, v, AuraInfo.name, AuraInfo.icon, AuraInfo.charges, AuraInfo.duration, AuraInfo.expirationTime, AuraInfo.sourceUnit, AuraInfo.spellId)
...
local function Watcher_SpellEvent(f)
	...
	local GCD = (F.IsClassic and 1.5) or GCDInfo.duration
	for i = 1, f.Info.NumButton do
		...
		if SpellID then
			...
			if SpellInfo then
				start, duration, enabled = SpellInfo.startTime, SpellInfo.duration, SpellInfo.isEnabled
			end

			local charges, maxCharges, chargeStart, chargeDuration
			if ChargeInfo then
				charges, maxCharges, chargeStart, chargeDuration = ChargeInfo.currentCharges, ChargeInfo.maxCharges, ChargeInfo.cooldownStartTime, ChargeInfo.cooldownDuration
			end
	...
	for i = 1, frame.Info.NumButton do
		if frame.Button[i] then
			...
			if SpellID then
				...
				local inRange = C_Spell.IsSpellInRange(name, "target") or 1
...
local function Bar_Update(frame, elapsed, ...)
	local GCD = (F.IsClassic and 1.5) or select(2,C_Spell.GetSpellCooldown(61304))
	for i = 1, frame.Info.NumButton do
...
local function FCS_Fade(frame, parent)
	local ShowFrame = 0
	for i = 1, frame.Info.NumButton do
...
local function FCS_OnEvent(self)
	...
	self: SetScript("OnEvent", function(self, event, ...)
		...
		FCS_Fade(self.BarWatcher, self)
		FCS_Fade(self.IconWatcher, self)
	end)
end
...
local function FCS_OnUpdate(self)
	...
	self: SetScript("OnUpdate", function(self, elapsed, ...)
		if Elapsed_Help >= 7*elapsed then
...
local function MEKA_FCS_Load()
	if Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_FCS.Enable then
		...
		if Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_FCS.Filter then	
			FCS_FILTER = true
		else
			FCS_FILTER = false
		end
...
local function MEKA_FCS_Toggle(arg1, arg2)
	if arg1 == "ON" then
	...
	Config = {
		...
		Sub = {
			[1] = {
				...
				Click = function(self, button)
					if Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_FCS.Filter then
						Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_FCS.Filter = false
						self.Text:SetText(L["OFF"])
						MEKA_FCS_Toggle("FILTER","OFF")
					else
						Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_FCS.Filter = true
						self.Text:SetText(L["ON"])
						MEKA_FCS_Toggle("FILTER","ON")
					end
				end,
				Show = function(self)
					if Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_FCS.Filter then
