...
local function Block_Create(frame)
	...
	local Bd1 = Block:CreateTexture(nil, "ARTWORK")
...
local function HealthPercent_Create(frame)
	...
	local Text = F.create_Font(HealthPercent, C.Font.Big, 24, nil, 0.9, "CENTER", "CENTER")
...
local function LeftFrame(frame)
	...
	Left.HealthValue = F.create_Font(Left.Block, C.Font.Num, 12, nil, 0.9, "RIGHT", "CENTER")
	Left.HealthValue: SetTextColor(F.Color(C.Color.Main1))
	Left.HealthValue: SetPoint("BOTTOMRIGHT", Left.Block, "RIGHT", -53, 0.5)
	Left.PowerValue = F.create_Font(Left.Block, C.Font.Num, 12, nil, 0.9, "RIGHT", "CENTER")
...
local function RightFrame(frame)
	...
	Right.HealthValue = F.create_Font(Right.Block, C.Font.Num, 12, nil, 0.9, "LEFT", "CENTER")
	Right.HealthValue: SetTextColor(F.Color(C.Color.Main1))
	Right.HealthValue: SetPoint("BOTTOMLEFT", Right.Block, "LEFT", 53, 0.5)
	Right.PowerValue = F.create_Font(Right.Block, C.Font.Num, 12, nil, 0.9, "LEFT", "CENTER")
...
local function RingBar1Num_Update(frame, per_value, hp_value)
	frame.HealthValue: SetText(F.FormatNum(hp_value, 1))
	frame.HealthPercent.Text: SetText(F.FormatPercent(per_value))
end
...
local function RingBar2Num_Update(frame, pp_per, pp_value, pp_type)
	if not pp_per then pp_per = 0 end
	pp_per = F.Clamp(pp_per, 0,1)
	if (not frame.Bar2.OldNumValue) or (frame.Bar2.OldNumValue ~= pp_per) then
		frame.Bar2.OldNumValue = pp_per
		local d,d1,d2,d3 = 0,0,0,0,0
		if pp_type and pp_type == 0 then
			d = pp_per * 100
		else
			d = F.Clamp(pp_value, 0,999)
		end
		d3,d2,d1 = F.BreakNums(d, 3)
		...
		frame.PowerValue: SetText(F.FormatNum(pp_value, 1))
	end
end
...
local function HUD_Target_Event(frame, event, ...)
	if event == "PLAYER_ENTERING_WORLD" or event == "PLAYER_TARGET_CHANGED" or event == "UNIT_FACTION" then
		...
		if UnitExists(frame.Unit) then
			...
			local isQuest = (not F.IsClassic) and UnitIsQuestBoss(frame.Unit)
			if isQuest then
...
local function HUD_Right_Update(frame)
	...
	frame.Right: RegisterUnitEvent("UNIT_HEALTH", "target")
	frame.Right: RegisterUnitEvent("UNIT_MAXHEALTH", "target")
	frame.Right: RegisterUnitEvent("UNIT_POWER_UPDATE", "target")
...
local MEKA_RINGHUD = F.IsClassic and CreateFrame("Frame", nil, E) or CreateFrame("Frame", nil, E, "MEKA_HUD_SlideTemplate")
...
local function MEKA_RingHUD_Player_HpUpdate(arg1,arg2,arg3)
	if arg1 == "Update" then
		RingBar1_Update(MEKA_RINGHUD.Left, arg2)
    elseif arg1 == "Event" then
		RingBar1Num_Update(MEKA_RINGHUD.Left, arg2,arg3)
    end
end

local function MEKA_RingHUD_Player_PpUpdate(arg1,arg2,arg3,arg4,arg5)
	if arg1 == "Update" then
		RingBar2_Update(MEKA_RINGHUD.Left, arg2)
    elseif arg1 == "Event" then
		RingBar2Num_Update(MEKA_RINGHUD.Left, arg2,arg3,arg5)
    end
end

local function MEKA_RingHUD_Player_AsUpdate(arg1,arg2)
	if arg1 == "Update" then
		RingBar4_Update(MEKA_RINGHUD.Left, arg2)
	end
end

local function MEKA_RingHUD_Player_MnUpdate(arg1,arg2)
	if arg1 == "Update" then
		RingBar3_Update(MEKA_RINGHUD.Left, arg2)
	end
end

local function MEKA_RingHUD_Pet_HpUpdate(arg1,arg2)
	if arg1 == "Update" then
		F.LoopBar6_Update(MEKA_RINGHUD.Left, arg2)
	end
end

local function MEKA_RingHUD_Pet_PpUpdate(arg1,arg2)
	if arg1 == "Update" then
		F.LoopBar6_Update(MEKA_RINGHUD.Left,nil, arg2)
	end
end

local function MEKA_RingHUD_Target_HpUpdate(arg1,arg2,arg3)
	if arg1 == "Update" then
		RingBar1_Update(MEKA_RINGHUD.Right, arg2)
    elseif arg1 == "Event" then
		RingBar1Num_Update(MEKA_RINGHUD.Right, arg2,arg3)
    end
end

local function MEKA_RingHUD_Target_PpUpdate(arg1,arg2,arg3,arg4,arg5)
	if arg1 == "Update" then
		RingBar2_Update(MEKA_RINGHUD.Right, arg2)
    elseif arg1 == "Event" then
		RingBar2Num_Update(MEKA_RINGHUD.Right, arg2,arg3,arg5)
    end
end

local function MEKA_RingHUD_Target_AsUpdate(arg1,arg2)
	if arg1 == "Update" then
		RingBar4_Update(MEKA_RINGHUD.Right, arg2)
	end
end
...
local function MEKA_RINGHUD_Load()
	if Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_HUD.State == "RING" then
		...
		tinsert(E.UBU.Player.HP, MEKA_RingHUD_Player_HpUpdate)
        tinsert(E.UBU.Player.PP, MEKA_RingHUD_Player_PpUpdate)
        tinsert(E.UBU.Player.AS, MEKA_RingHUD_Player_AsUpdate)
        tinsert(E.UBU.Player.MN, MEKA_RingHUD_Player_MnUpdate)
		tinsert(E.UBU.Pet.HP, MEKA_RingHUD_Pet_HpUpdate)
		tinsert(E.UBU.Pet.PP, MEKA_RingHUD_Pet_PpUpdate)

		RightFrame(MEKA_RINGHUD)
		tinsert(E.UBU.Target.HP, MEKA_RingHUD_Target_HpUpdate)
        tinsert(E.UBU.Target.PP, MEKA_RingHUD_Target_PpUpdate)
