...
local function MEKA_PlayerFrame_Create(frame)
	local Health = CreateFrame("Button", "Quafe_MEKA "..L['PLAYER_FRAME'].." Health", frame, "SecureUnitButtonTemplate")
	Health.Unit = "player"
	...
	local Power = CreateFrame("Button", "Quafe_MEKA "..L['PLAYER_FRAME'].." Power", frame, "SecureUnitButtonTemplate")
	Power.Unit = "player"
	...
	local Pet = CreateFrame("Button", "Quafe_MEKA "..L['PET_FRAME'], frame, "SecureUnitButtonTemplate")
	Pet.Unit = "pet"
	Pet: SetSize(86,94)
...
local function UpdateManaNum(frame, value)
	if not value then value = 0 end
...
local function Update_PlayeButtonrColor(frame, color)
	frame.Bar: SetVertexColor(F.Color(color))
	if frame.BarEx then
		frame.BarEx: SetVertexColor(F.Color(color))
	end
	if frame.BarEnd then
		frame.BarEnd: SetVertexColor(F.Color(color))
	end
	if frame.PhIcon then
		frame.PhIcon: SetVertexColor(F.Color(color))
	end
	if frame.PhBar then
		frame.PhBar: SetVertexColor(F.Color(color))
	end
	if frame.PhBg then
		frame.PhBg: SetVertexColor(F.Color(color))
	end
...
local GarrsionTabButtons = {
	[1] = {
		ID = Enum.GarrisonType.Type_6_0_Garrison,
		Icon = "Interface\\ICONS\\Achievement_Level_100",
	},
	[2] = {
		ID = Enum.GarrisonType.Type_7_0_Garrison,
		Icon = "Interface\\ICONS\\Achievement_Level_110",
	},
	[3] = {
		ID = Enum.GarrisonType.Type_8_0_Garrison,
		Icon = "Interface\\ICONS\\INV_level120",
	},
	[4] = {
		ID = Enum.GarrisonType.Type_9_0_Garrison,
		Icon = "Interface\\ICONS\\INV_level120",
	},
}
...
local function Extra_Garrison(frame)
	...
	local Icon = Garrison: CreateTexture(nil, "ARTWORK")
...
local function Extra_Queue(frame)
	...
	local Icon = Queue: CreateTexture(nil, "ARTWORK")
...
local function MEKA_MinimapFrame_Create(frame)
	...
	local DungeonDifficulty = F.DungeonDifficulty_Template(Main)
	DungeonDifficulty: SetPoint("BOTTOMRIGHT", Main, "TOPRIGHT", -80, 40)
...
local function MEKA_PlayerFrame_HpUpdate(arg1,arg2,arg3,arg4)
    if arg1 == "Update" then
		Update_RingBar1(MEKA_PlayerFrame.Health, arg2)
    elseif arg1 == "Event" then
		Update_InfoNum(MEKA_PlayerFrame.Health, arg3)
		Update_InfoMaxNum(MEKA_PlayerFrame.Health, arg4)
    end
end

local function MEKA_PlayerFrame_PpUpdate(arg1,arg2,arg3,arg4)
    if arg1 == "Update" then
        Update_RingBar1(MEKA_PlayerFrame.Power, arg2)
    elseif arg1 == "Event" then
        Update_InfoNum(MEKA_PlayerFrame.Power, arg3)
		Update_InfoMaxNum(MEKA_PlayerFrame.Power, arg4)
    end
end

local function MEKA_PlayerFrame_MnUpdate(arg1,arg2)
    if arg1 == "Update" then
        if MEKA_PlayerFrame.ShowMana then
			Update_RingBar2(MEKA_PlayerFrame.Mana, arg2)
		end
	elseif arg1 == "Event" then
		if MEKA_PlayerFrame.ShowMana then
			UpdateManaNum(MEKA_PlayerFrame.Mana, arg2)
		end
    end
end

local function MEKA_PetFrame_HpUpdate(arg1,arg2,arg3,arg4)
    if arg1 == "Update" then
        if MEKA_PlayerFrame.ShowPet then
			Update_RingBar2(MEKA_PlayerFrame.Mana, arg2)
		end
    elseif arg1 == "Event" then
		if MEKA_PlayerFrame.ShowPet then
...
local function MEKA_PlayerFrame_Load()
	if Quafe_DB.Profile[Quafe_DBP.Profile].MEKA_PlayerFrame.Enable then
		...
		tinsert(E.UBU.Player.HP, MEKA_PlayerFrame_HpUpdate)
        tinsert(E.UBU.Player.PP, MEKA_PlayerFrame_PpUpdate)
        tinsert(E.UBU.Player.AS, MEKA_PlayerFrame_AsUpdate)
        tinsert(E.UBU.Player.MN, MEKA_PlayerFrame_MnUpdate)
		tinsert(E.UBU.Pet.HP, MEKA_PetFrame_HpUpdate)
...
local function MEKA_PlayerFrame_Toggle(arg)
	if arg == "ON" then
		...
		Player_Event(MEKA_PlayerFrame, "PLAYER_ENTERING_WORLD")
		F.UBGU_ForceUpdate()
	elseif arg == "OFF" then
