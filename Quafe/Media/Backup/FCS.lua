...
local Ring_Artwork = function(f, size)
	...
	f.Ring: SetTexture(F.Path("Ring16_2"))
...
local Create_Ring = function(f, size1, size2)
	...
	Bg1: SetTexture(FF.Path("Ring32_Bg"))
...
local function Point_Frame(f)
	...
	for i = 1,6 do
		...
		f["Point"..i].Bar: SetTexture(F.Path("Ring16_Bg"))
		...
		f["Point"..i].BarEx: SetTexture(F.Path("Ring16_Bg2"))
		...
		f["Point"..i].BarBg: SetTexture(F.Path("Ring16_Bg"))
...
local function Pita_Bar1(f, p, angle)
	...
	f.Bg: SetTexture(F.Path("Pita_Bar\\Pita_Bar_72"))
	...
	f.Bar: SetTexture(F.Path("Pita_Bar\\Pita_Bar_72"))
	...
	for i = 1,3 do
		...
		f.Num[i]: SetTexture(F.Path("N12\\N".."9"))
	...
	for i = 1,2 do
		...
		f.Count[i]: SetTexture(F.Path("N12\\N".."9"))
...
local function Ring_Bar1(f, p, angle)
	...
	f.Bg = f:CreateTexture(nil, "BACKGROUND")
	f.Bg: SetTexture(F.Path("HUD_Ring_BarBg"))
	...
	f.BarBg: SetTexture(F.Path("Ring_Bar\\Ring_Bar_36"))
	...
	f.Bar = f:CreateTexture(nil, "ARTWORK")
	f.Bar: SetTexture(F.Path("Ring_Bar\\Ring_Bar_36"))
	f.Bar: SetVertexColor(F.Color(C.Color.B1))
	f.Bar: SetAlpha(0.9)
	f.Bar: SetSize(64,64)
	...
	for i = 1,3 do
		f.Count[i] = f:CreateTexture(nil, "OVERLAY")
		f.Count[i]: SetTexture(F.Path("N16\\N".."9"))
		f.Count[i]: SetVertexColor(F.Color(C.Color.B1))
		f.Count[i]: SetAlpha(0)
		f.Count[i]: SetSize(32,32)
		F.RotateTexture(f.Count[i], angle)
...
local function Icon_Bar1(f, p, i)
	...
	Bar: SetStatusBarTexture(F.Path("StatusBar\\Flat"))
...
local function Watcher_BarUpdate(f, d, show)
	...
	if f.Style == "Bar" then
		...
		d = floor(31*d + 0.5) * 2
		f.Bar: SetTexture(F.Path("Pita_Bar\\Pita_Bar_"..d))
	elseif f.Style == "Count" then
		...
		d = floor(36*d + 0.5)
		f.Bar: SetTexture(F.Path("Ring_Bar\\Ring_Bar_"..d))
...
local function Watcher_NumUpdate(f, value)
	if f.Style == "Bar" then
		...
		elseif v1 == 0 and v2 ~= "p" then
			f.Num[1]: SetTexture(F.Path("N12\\N"..v2))
			f.Num[2]: SetTexture(F.Path("N12\\N"..v3))
			f.Num[1]: SetAlpha(1)
			f.Num[2]: SetAlpha(1)
			f.Num[3]: SetAlpha(0)
		else
			f.Num[1]: SetTexture(F.Path("N12\\N"..v1))
			f.Num[2]: SetTexture(F.Path("N12\\N"..v2))
			f.Num[3]: SetTexture(F.Path("N12\\N"..v3))
			f.Num[1]: SetAlpha(1)
			f.Num[2]: SetAlpha(1)
			f.Num[3]: SetAlpha(1)
		end
...
local function Watcher_CountUpdate(f, value, zero)
	if f.Style == "Bar" then
		...
		elseif v1 == 0 then
			f.Count[1]: SetTexture(F.Path("N12\\N"..v2))
			f.Count[1]: SetAlpha(1)
			f.Count[2]: SetAlpha(0)
		else
			f.Count[1]: SetTexture(F.Path("N12\\N"..v2))
			f.Count[2]: SetTexture(F.Path("N12\\N"..v1))
			f.Count[1]: SetAlpha(1)
	...
	elseif f.Style == "Count" then
		...
		elseif v1 == 0 then
			f.Count[3]: SetTexture(F.Path("N16\\N"..v2))
			f.Count[1]: SetAlpha(0)
			f.Count[2]: SetAlpha(0)
			f.Count[3]: SetAlpha(1)
		else
			f.Count[1]: SetTexture(F.Path("N16\\N"..v1))
			f.Count[2]: SetTexture(F.Path("N16\\N"..v2))
			f.Count[1]: SetAlpha(1)
			f.Count[2]: SetAlpha(1)
			f.Count[3]: SetAlpha(0)
		end
